...
namespace RadiusDimensionMover
{
    ...
    internal static class WindowCapture
    {
        ...
        /// <summary>
        /// Jaka CZĘŚĆ nowo narysowanego wymiaru wylądowała NA CZYMŚ, co już
        /// tam było - mierzone bezpośrednio, piksel w piksel, bez zgadywania
        /// pozycji tekstu.
        ///
        /// Zwraca (overlap, changed): `changed` to liczba pikseli, które się
        /// zmieniły między zrzutami, a `overlap` to ułamek tych zmienionych
        /// pikseli, w których PRZED zmianą już coś było narysowane (czyli
        /// wymiar nałożył się na istniejącą treść). Rysunek w Tekli jest na
        /// czarnym tle, więc "coś tu było" = piksel jaśniejszy od tła.
        ///
        /// Zastępuje wcześniejsze liczenie zajętości w kwadracie wokół
        /// środka ciężkości różnicy zrzutów: ten środek wypadał POMIĘDZY
        /// starą i nową pozycją wymiaru (zmieniają się oba miejsca), więc
        /// odczyty zajętości dotyczyły w połowie przypadków zupełnie innego
        /// miejsca niż tekst - stąd chaotyczne wyniki 0,00 tam, gdzie tekst
        /// wyraźnie leżał na otworze.
        ///
        /// Piksele ramki/prowadnic (patrz IsFrameOrGuideColor) nie liczą się
        /// jako istniejąca treść.
        /// </summary>
        public static (double overlap, int changed) GetOverlapWithExisting(
            Bitmap before, Bitmap after, int changeThreshold = 25, int backgroundThreshold = 30)
        {
            int w = Math.Min(before.Width, after.Width);
            int h = Math.Min(before.Height, after.Height);
            var rect = new Rectangle(0, 0, w, h);
            ...
            for (int y = 0; y < h; y++)
            {
                ...
                for (int x = 0; x < w; x++)
                {
                    int i1 = row1 + x * 4;
                    int i2 = row2 + x * 4;
                    int b1 = bytes1[i1], g1 = bytes1[i1 + 1], r1 = bytes1[i1 + 2];
                    int b2 = bytes2[i2], g2 = bytes2[i2 + 1], r2 = bytes2[i2 + 2];

                    if (Math.Abs(b1 - b2) + Math.Abs(g1 - g2) + Math.Abs(r1 - r2) <= changeThreshold)
                    {
                        continue;
                    }

                    // Interesują nas tylko piksele, w których wymiar POJAWIŁ
                    // się (po zmianie coś jest) - miejsca, z których zniknął,
                    // nic nie mówią o kolizji.
                    bool contentAfter = b2 > backgroundThreshold || g2 > backgroundThreshold || r2 > backgroundThreshold;
                    if (!contentAfter)
                    {
                        continue;
                    }
                    ...
                    bool contentBefore = (b1 > backgroundThreshold || g1 > backgroundThreshold || r1 > backgroundThreshold)
                        && !IsFrameOrGuideColor(r1, g1, b1);
                    if (contentBefore)
                    {
                        onExisting++;
                    }
            ...
            double overlap = changed > 0 ? (double)onExisting / changed : 1.0;
